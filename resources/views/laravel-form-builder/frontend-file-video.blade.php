...
@foreach ($options['files'] as $file)
    ...
    <div class="media-{{ $file['id'] }}-container mb-2" x-data="{ deleted: false }" x-show="!deleted">
        <video class="rounded-lg w-full max-w-md" controls src="{{ $file['public_path'] }}"></video>
        <div class="flex items-center gap-3 mt-2">
            <button type="button"
                    ...
                    x-on:click="if(confirm('{{ trans('motor-backend::backend/global.delete_question') }}')) { deleted = true; document.querySelector('input[name=\'delete_media_{{ $file['id'] }}\']').value = 1; }">
            ...
            <div>
                ...
                <span class="block text-sm text-text-muted">{{ trans('motor-backend::backend/global.uploaded') }} {{ $file['created_at'] }}</span>
            </div>
        </div>
    </div>
@endforeach
