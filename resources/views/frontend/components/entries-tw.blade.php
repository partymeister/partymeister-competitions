<h4 class="text-lg font-bold mb-4 flex items-center justify-between">
    ...
    <a href="{{route('frontend.pages.index', ['slug' => $component->entry_edit_page->full_slug])}}" class="inline-flex items-center justify-center rounded-lg bg-success px-3 py-1.5 text-xs font-semibold text-body hover:bg-success/90 transition-colors">Upload entry</a>
</h4>
@if ($entries->count() == 0)
                    ...
                    <div class="flex w-full">
                        @if ($entry->competition->upload_enabled || $entry->upload_enabled)
                            <a href="{{route('frontend.pages.index', ['slug' => $component->entry_edit_page->full_slug]) }}?entry_id={{$entry->id}}"
                               class="flex-1 inline-flex items-center justify-center bg-accent px-3 py-1.5 text-xs font-semibold text-body hover:bg-accent-hover transition-colors rounded-l-lg">Edit</a>
                        @endif
                        @if ($entry->competition->competition_type->has_screenshot)
                            <a href="{{route('frontend.pages.index', ['slug' => $component->entry_screenshots_page->full_slug])}}?entry_id={{$entry->id}}"
                               class="flex-1 inline-flex items-center justify-center bg-accent px-3 py-1.5 text-xs font-semibold text-body hover:bg-accent-hover transition-colors">Update screenshot</a>
                        @endif
                        <a href="{{route('frontend.pages.index', ['slug' => $component->entry_detail_page->full_slug])}}?entry_id={{$entry->id}}"
                           class="flex-1 inline-flex items-center justify-center bg-accent px-3 py-1.5 text-xs font-semibold text-body hover:bg-accent-hover transition-colors rounded-r-lg">Show</a>
                    </div>
                    <a href="{{route('frontend.pages.index', ['slug' => $component->entry_comments_page->full_slug])}}?entry_id={{$entry->id}}"
                       class="w-full inline-flex items-center justify-center rounded-lg px-3 py-1.5 text-xs font-semibold text-body transition-colors mt-2 @if ($entry->new_comments > 0) bg-accent hover:bg-accent-hover @else bg-surface-raised font-medium text-text hover:text-heading @endif">Messages @if ($entry->new_comments > 0)
