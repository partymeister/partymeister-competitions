...
class EntryForm extends Form
{
    ...
    public function buildForm()
    {
        ...
        $this->add('competition_id', 'select2', [
                ...
                'choices'     => Competition::orderBy('sort_position')->pluck('name', 'id')->toArray()
            ])
             ->add('reload_on_change', 'hidden', [ 'attr' => [ 'id' => 'reload_on_change' ] ])
             ->add(
                 'sort_position',
                 'text',
                 [ 'label' => trans('partymeister-competitions::backend/entries.sort_position') ]
             )
             ->add(
                 'title',
                 'text',
             ...
             ->add(
                 'is_prepared',
                 'checkbox',
                 [ 'label' => trans('partymeister-competitions::backend/entries.is_prepared') ]
             )
             ->add('status', 'select', [
             ...
             ->add(
                 'upload_enabled',
                 'checkbox',
                 [ 'label' => trans('partymeister-competitions::backend/entries.upload_enabled') ]
             )
            ...
            ->add(
             ...
             ->add(
                 'file',
                 'file_file',
                 [ 'label' => trans('partymeister-competitions::backend/entries.file'), 'model' => Entry::class ]
             );
        ...
        if (isset($data['competition'])) {
            ...
            if ($data['competition']->competition_type->has_audio) {
                $this->add(
                    'audio',
                    'file_audio',
                    [ 'label' => trans('partymeister-competitions::backend/entries.audio'), 'model' => Entry::class ]
                );
            }
            if ($data['competition']->competition_type->has_filesize) {
                $this->add(
                    'filesize',
                    'text',
                    [ 'label' => trans('partymeister-competitions::backend/entries.filesize') ]
                );
            }
            if ($data['competition']->competition_type->has_platform) {
                $this->add(
                    'platform',
                    'text',
                    [ 'label' => trans('partymeister-competitions::backend/entries.platform') ]
                );
            }
            if ($data['competition']->competition_type->has_running_time) {
                $this->add(
                    'running_time',
                    'text',
                    [ 'label' => trans('partymeister-competitions::backend/entries.running_time') ]
                );
            }
            if ($data['competition']->competition_type->has_remote_entries) {
                $this->add(
                    'is_remote',
                    'checkbox',
                    [ 'label' => trans('partymeister-competitions::backend/entries.is_remote') ]
                );
            }
            if ($data['competition']->competition_type->has_recordings) {
                $this->add(
                    'is_recorded',
                    'checkbox',
                    [ 'label' => trans('partymeister-competitions::backend/entries.is_recorded') ]
                );
            }
